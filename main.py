...
async def notify_all(context: ContextTypes.DEFAULT_TYPE, old, data):
    ...
    old = old["Data"]
    for x in data:
        if x["Item"] not in list(old.values()):
            Details = x['Details']
...
async def main():
    global subscribers
    subscribers = load_subscribers()
    app = ApplicationBuilder().token("7944426745:AAFR4cnfgavqpd3B1PH3A_IgibUG0kx9zeI").build()
    ...
    scheduler.add_job(scrape, "interval", minutes=1, args=[app.bot])
    scheduler.start()
